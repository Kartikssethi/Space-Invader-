...
def check_camera_permission():
    try:
        ...
        if not cap.isOpened():
            ...
            call(["open",
                  "x-apple.systempreferences:com.apple.preference.security?Privacy_Camera"])  # Open system preferences
            return False
        cap.release()
        return True
        ...
        try:
            ...
            while running.value:
                ...
                if results.multi_hand_landmarks:
                    # Check for restart gesture (both hands up)
                    if len(results.multi_hand_landmarks) == 2:
                        ...
                        for hand_landmarks in results.multi_hand_landmarks:
                            ...
                            if middle_tip.y >= wrist.y:
                                hands_up = False
                                break
                        restart.value = hands_up

                        if hands_up:
                            cv2.putText(image, "RESTART!", (10, 90),
                    ...
                    # Add visual indicator for hand raised status
...
def restart_game():
    ...
    # Destroy game over text if it exists
    for entity in scene.entities:
        if isinstance(entity, Text) and (entity.text.startswith('Game Over') or entity.text.startswith('Final Score')):
            destroy(entity)
...
def input(key):
    ...
    if game_over:
        if key == "r":  # Check if the 'r' key is pressed
    ...
    elif key == "space" and bullet_count > 0:  # Only shoot if bullets are available
        Audio('assets/laser_sound.wav')
        bullet = Bullet()
        bullets.append(bullet)
        bullet_count -= 1  # Reduce the bullet count
...
# Display score
score_text = Text(text='Score: 0', position=(-0.65, 0.4), origin=(0, 0), scale=2, color=color.violet, background=True)

# Display ammo count
ammo_text = Text(text=f"Ammo: {bullet_count}", position=(0.65, 0.4), origin=(0, 0), scale=2, color=color.magenta,
                 background=True)
